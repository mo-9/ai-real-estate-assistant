...
- For direct API calls, include the `X-API-Key` header (see API Reference). The web app proxies API calls server-side, so the browser does not embed the API key.
...
- Click a cluster bubble to zoom into that area and reveal individual markers.
