...
def test_queue_price_drop_alert_serializes_property_and_processes(tmp_path):
    ...
    am.queue_alert(Alert(alert_type=AlertType.PRICE_DROP, user_email="user@example.com", data=drop, property_id="p1"))
...
def test_process_pending_alerts_respects_predicate(tmp_path):
    ...
    prop = make_prop("p1", "Krakow", 900, 2)
    drop = {"property": prop, "old_price": 1000, "new_price": 900, "percent_drop": 10.0, "savings": 100}
    am.queue_alert(Alert(alert_type=AlertType.PRICE_DROP, user_email="user@example.com", data=drop, property_id="p1"))

    with patch.object(EmailService, "send_email", return_value=True) as send_mock:
        sent_count, sent_alerts = am.process_pending_alerts_with_result(should_send=lambda _a: False)
        assert sent_count == 0
        assert sent_alerts == []
...
def test_send_price_drop_alert_accepts_property_dict(tmp_path):
    ...
    drop = {"property": prop_dict, "old_price": 1000, "new_price": 900, "percent_drop": 10.0, "savings": 100}
...
def test_queue_price_drop_alert_serialization_uses_model_dump_when_available(monkeypatch, tmp_path):
    ...
    def _fake_model_dump(self, **_kwargs):
        return {"id": self.id, "city": self.city, "price": self.price, "rooms": self.rooms, "bathrooms": self.bathrooms}
...
def test_queue_price_drop_alert_serialization_falls_back_to_json(monkeypatch, tmp_path):
    ...
    def _fake_json(self, **_kwargs):
        return json.dumps(
            {"id": self.id, "city": self.city, "price": self.price, "rooms": self.rooms, "bathrooms": self.bathrooms}
        )
